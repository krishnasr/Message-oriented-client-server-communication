...
class switch_process:
    def __init__(self, process,client_connect):
        self.process=process
        self.client_connect=client_connect
    def upload_process(self):
        filename=self.client_connect.recv(1024).decode(message_format) # receiving the file name to the server for upload
        curr_file=open(filename,"wb")
        curr_filedata=self.client_connect.recv(1024)
        flag="available"
        while flag=="available":
            curr_filedata=self.client_connect.recv(1024) # receiving the file data to the server
            if curr_filedata:
    ...
    def download_process(self):
        file_path=self.client_connect.recv(3072).decode(message_format) #receiving the server's file path to get the list of files that are in the server
        files_list=os.listdir(file_path)
        files_list.append("cancel")
        for files in files_list:
            print(files)
            self.client_connect.send(files.encode(message_format)) # sending the file name that are in the server
        filename=self.client_connect.recv(1024).decode(message_format)
        curr_file=open(filename,"rb")
        flag="available"
        while flag=="available":
            curr_filedata=curr_file.read(1024)
            if curr_filedata:
                self.client_connect.send(curr_filedata) # sending the file data to the client to download
            else:
                flag="notavailable"
        curr_file.close()
    ...
    def delete_process(self):
        ...
        for files in files_list:
            print(files)
            self.client_connect.send(files.encode(message_format)) # sending the file name that are in the server
        filename=self.client_connect.recv(1024).decode(message_format) # receiving the filename that need to be deleted
        os.remove(filename)
    ...
    def rename_process(self):
        ...
        for files in files_list:
            print(files)
            self.client_connect.send(files.encode(message_format)) # sending the file name that are in the server
        filename=self.client_connect.recv(1024).decode(message_format) # receiving the filename that need to be renamed
        file_newname=self.client_connect.recv(1024).decode(message_format) # receiving new file name
        ...
        for files in files_list:
            print(files)
            self.client_connect.send(files.encode(message_format))
